...
class DiamondRushSolver:
    ...
    def _discover_button_door_mapping(self):
        """Discover button-door relationships dynamically"""
        # In a real implementation, this would be discovered through gameplay
        mapping = {}
        if len(self.all_buttons) > 0 and len(self.all_doors) > 0:
            # For demo, create a simple 1:1 mapping
            for i, button in enumerate(self.all_buttons):
                if i < len(self.all_doors):
                    door = list(self.all_doors)[i]
                    mapping[button] = {door}
        return mapping
    ...
    def _create_new_state(self, state: GameState, new_pos, current_pose):
        ...
        # Update doors
        new_doors_open = state.doors_open   #self._update_doors(new_buttons_pressed, state.rocks)
